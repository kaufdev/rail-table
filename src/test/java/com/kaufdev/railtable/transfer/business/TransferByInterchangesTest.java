...
class TransferByInterchangesTest {
    ...
    public void interchangeCreatedFromCoupleOfSectionsShouldHaveProperProperties(){
        ...
        Section section1 = new Section(KRK_STATION,WAW_STATION, expectedStartingTime, LocalDateTime.MIN,0);
        Section section2 = new Section(WAW_STATION,POZ_STATION, LocalDateTime.MIN, LocalDateTime.MIN,0);
        Section section3 = new Section(POZ_STATION,LIZ_STATION, LocalDateTime.MIN, expectedEndingTime,0);
        ReflectionTestUtils.setField(section1,"transfer", transfer);
        ReflectionTestUtils.setField(section2,"transfer", transfer);
        ReflectionTestUtils.setField(section3,"transfer", transfer);

        TransferByInterchanges subject = new TransferByInterchanges(List.of(section1,section2, section3));

        //when
        assertThat(subject.getInterchanges()).hasSize(1).flatExtracting("startStation","endStation","outboundTime","arrivalTime")
                .containsExactly(StationAssembler.assemble(KRK_STATION),StationAssembler.assemble(LIZ_STATION),expectedStartingTime,expectedEndingTime);
    }
