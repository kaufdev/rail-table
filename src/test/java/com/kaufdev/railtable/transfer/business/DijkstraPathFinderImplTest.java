...
class DijkstraPathFinderImplTest {
    ...
    public void whenPathIsImpossibleEmptyOptionalShouldBeReturned(){
        ...
        //when
        Optional<List<Long>> pathOptional = pathFinder.getPath(network, "WAW", "POZ");
        //then
    ...
    public void whenTwoMultiPathsAsPossibleQuickerShouldBeChosen(){
        ...
        //when
        Optional<List<Long>> path = pathFinder.getPath(network, "KRK", "POZ");

        //then
        org.assertj.core.api.Assertions.assertThat(path.get()).containsExactly(3L,4L);
    }
}